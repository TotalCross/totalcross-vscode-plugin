...
export interface MixedProjectLayout {
    buildTool: 'mixed';
    root: string;
}
...
/** Detects mixed roots explicitly rather than guessing whether Maven or Gradle is authoritative. */
export async function detectProjectLayout(root: string, platform: NodeJS.Platform): Promise<ProjectLayout | MixedProjectLayout | undefined> {
...
export async function resolveGradleApplicationName(root: string, fallback: string): Promise<ApplicationNameResolution> {
    try {
        const metadata = JSON.parse(await fs.readFile(path.join(root, '.totalcross', 'project.json'), 'utf8'));
        if (metadata && typeof metadata.applicationName === 'string' && metadata.applicationName) {
            return {name: metadata.applicationName};
        }
    } catch (_) {
        // Metadata is optional and only generated by the Maven conversion workflow.
    }
