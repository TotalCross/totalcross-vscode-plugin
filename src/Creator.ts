...
exports.createNewProject = async function(context: vscode.ExtensionContext) {
    ...
    let groupID = await showInputBox('Enter project GroupId', 'example: com.totalcross', validators.groupId);
    if(!groupID) {return;}
    let artifactID = await showInputBox('Enter project ArtifactId', 'example: MyFirstApp', null);
