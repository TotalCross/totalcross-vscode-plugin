...
export class RegisterHandler {
     ...
     */
    async doRegister() : Promise<Response>{
        return new Promise(async function(resolve, reject) {
            
            let firstName = await showInputBox(
                'First name',
                'John', 
                registerValidator.validateNonEmpty('First name')
                );
            ...
            let lastName = await showInputBox(
                'Last name',
                'Doe', 
                registerValidator.validateNonEmpty('Last name')
                );
            ...
            let country = await vscode.window.showQuickPick (
                Object.keys(isoCountries),
                {
                    placeHolder: 'Choose your country (example: BR)'
                }
            );
            ...
            let username = await showInputBox(
                'User name',
                'johnDoe1991', 
                registerValidator.validateNonEmpty('User name')
                );
            ...
            let password = await showInputBoxPassword(
                'Password',
                'password', 
                registerValidator.validateNonEmpty('Password')
                );
            ...
            let password2 = await showInputBoxPassword(
                'Confirm your password',
                'password', 
                (text: string) => {
                    return text === password? null: "Password confirmation doesn't match";
                }
                );    
            ...
            myStatusBarItem.text = `$(sync~spin) TotalCross Register Request...`;
